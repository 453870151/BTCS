...
const Contracts_Address = {
  56: {
    ...
    // BTZK_MachineMall
    BTZKMachineMallAddress: '0x1dcf9696556271b5d85e5420a7fa31e69a6ab560',
    // BTZK_Machine
    ...
    BTZKTokenRewardPoolAddress: '0x442321685602626F78C1899D13FC6764b4404774'
  },
  97: {
...
class ContractShenma {
  ...
  static async mintParams(address, BTZKMachineMallAddress, BTZKMachineAddress) {
    try {
      ...
      for (let i = 0; i <= max; i++) {
        // 封装成 Promise
        // 查询矿机列表
        const queryPromise = contract.methods.mintParams(i).call().then(data => ({
          type: i,
          success: true,
  ...
  static async mintMachine(address, tokenContract, contractsAddress, price, priceBig, electricityPriceBig, type, installmentValue, billFeeValue, isMintSwitch, mintToken) {
    try {
      ...
      if (installmentValue === 0) {
        ...
        if (Number(nativeBalance) < Number(priceBig / 10)) {
          var _ContractShenma$i18n5;
          ContractShenma.message.error((_ContractShenma$i18n5 = ContractShenma.i18n) == null ? void 0 : _ContractShenma$i18n5.t('BNB Insufficient balance'));
          return;
        }
      }
      if (Number(balanceOf) < Number(electricityPriceBig * billFeeValue)) {
        var _ContractShenma$i18n6;
        ContractShenma.message.error((_ContractShenma$i18n6 = ContractShenma.i18n) == null ? void 0 : _ContractShenma$i18n6.t('Insufficient balance'));
        return;
      }
      if (isMintSwitch && mintToken != null && mintToken.address) {
        ...
        if (Number(mintBalanceOf) < 1) {
          ...
          ContractShenma.message.error((_ContractShenma$i18n7 = ContractShenma.i18n) == null ? void 0 : _ContractShenma$i18n7.t('Please contact the club to queue for a number'));
          return;
        }
      }
      ...
      ContractShenma.message.success((_ContractShenma$i18n8 = ContractShenma.i18n) == null ? void 0 : _ContractShenma$i18n8.t('Activation successful'));
      return true;
    } catch (error) {
      console.log("error===>", error);
      return false;
  ...
  static async charge(address, tokenContract, contractsAddress, item, monthValue) {
    try {
      ...
      if (Number(balanceOf) < Number(amount)) {
        ...
        ContractShenma.message.error((_ContractShenma$i18n9 = ContractShenma.i18n) == null ? void 0 : _ContractShenma$i18n9.t('Insufficient balance'));
        return;
      }
      const contract = new contractShenmaUtil_web3.eth.Contract(BTZKMachineMallAbi_namespaceObject, contractsAddress);
      const gas = await contractShenmaUtil_web3.eth.getGasPrice();
      ...
      ContractShenma.message.success((_ContractShenma$i18n0 = ContractShenma.i18n) == null ? void 0 : _ContractShenma$i18n0.t('Renewal successful'));
      return true;
    } catch (error) {
      console.log("error===>", error);
      return false;
  ...
  static async renewal(address, tokenContract, contractsAddress, item, monthValue, amount) {
    try {
      ...
      if (Number(nativeBalance) < Number(bnbPriceBig)) {
        ...
        ContractShenma.message.error((_ContractShenma$i18n1 = ContractShenma.i18n) == null ? void 0 : _ContractShenma$i18n1.t('BNB Insufficient balance'));
        return;
      }
      if (Number(balanceOf) < Number(amount)) {
        var _ContractShenma$i18n10;
        ContractShenma.message.error((_ContractShenma$i18n10 = ContractShenma.i18n) == null ? void 0 : _ContractShenma$i18n10.t('Insufficient balance'));
        return;
      }
      const contract = new contractShenmaUtil_web3.eth.Contract(BTZKMachineMallAbi_namespaceObject, contractsAddress);
      const gas = await contractShenmaUtil_web3.eth.getGasPrice();
      ...
      ContractShenma.message.success((_ContractShenma$i18n11 = ContractShenma.i18n) == null ? void 0 : _ContractShenma$i18n11.t('Repayment successful'));
      return true;
    } catch (error) {
      console.log("error===>", error);
      return false;
  ...
  static async burn(address, tokenId, contractsAddress) {
    try {
      ...
      ContractShenma.message.success((_ContractShenma$i18n12 = ContractShenma.i18n) == null ? void 0 : _ContractShenma$i18n12.t('Miner destroyed successfully'));
      return true;
    } catch (error) {
      console.log("error===>", error);
      return false;
  ...
  static async claim(address, contractAddress, sender, token, amount, orderId, signature) {
    try {
      ...
      ContractShenma.message.success((_ContractShenma$i18n13 = ContractShenma.i18n) == null ? void 0 : _ContractShenma$i18n13.t('Withdraws successful'));
      return true;
    } catch (error) {
      console.log("error===>", error);
      return false;
...
const MiningProvider = _ref => {
  ...
  const [coinBalance, setCoinBalance] = (0,react.useState)(0); // 体验币余额
  ...
  // 查询体验币余额
  const handleCoinBalance = async () => {
    const balance = await ContractERC20.balanceOf(address, ContractsAddress.CoinTokenAddress);
...
instance2.interceptors.request.use(function (config) {
  // 添加认证token
  const accessToken = getCookie('BAccessToken');
...
// export const axiosUrl = interfaceUrl() ? 
// 	"https://openbtc.app/app-api" : "http://192.168.90.68:48080/app-api";
...
const UserProvider = _ref => {
  ...
  const [accessToken, setAccessToken] = (0,react.useState)(null);
  const [isInitialized, setIsInitialized] = (0,react.useState)(false);
  const prevAddressRef = (0,react.useRef)(null);
  const isSigningRef = (0,react.useRef)(false);
  ...
  const handleNoticeLatest = async () => {
    const latestId = getCookie('BTCSNoticeLatest');
    http.get(`${axiosUrl}/common/notice/latest`).then(function (res) {
      if (res.code === 0) {
        const data = res.data;
        if (data) {
          setCookie('BTCSNoticeLatest', data.id);
  ...
  const handleUserInfo = async () => {
    http2.get(`${axiosUrl}/assets/info/owner?address=${address}`).then(function (res) {
      if (res.code === 0) {
        const data = res.data;
  ...
  const handleAssetsInfo = async (assetsType, id, date) => {
    ...
    http2.get(`${axiosUrl}/assets/info/change-record?address=${address}&pageSize=${pageSize}&assetsType=${assetsType}&lastId=${id}&date=${date}`).then(function (res) {
      if (res.code === 0) {
        const data = res.data;
        ...
        message/* default */.Ay.error(t("Please refresh the page to sign"));
      }
    }).catch(function (error) {
      console.log("error===>", error);
    });
  ...
  const handleAssetsWithdraw = async assetsType => {
    ...
    http2.get(`${axiosUrl}/assets/withdraw/get-owner?address=${address}&assetsType=${assetsType}`).then(function (res) {
      if (res.code === 0) {
        const data = res.data;
  ...
  (0,react.useEffect)(() => {
    // 当 address 发生变化时，清空所有状态
    if (prevAddressRef.current && prevAddressRef.current !== address) {
      // 清空 token
      setAccessToken(null);
      // 清空初始化标记
      setIsInitialized(false);
      // 可选：清空 cookie 中的 token
      setCookie('BAccessToken', '', -1); // 设置过期时间为过去
      isSigningRef.current = false; // 重置签名标记
    }
    prevAddressRef.current = address;
    if (!address) return;
    const token = getCookie('BAccessToken');
    setAccessToken(token);
    if (token) {
      if (!isInitialized || token !== accessToken) {
  ...
  const handleLogin = async loginForm => {
    http.post(`${axiosUrl}/user/auth/login`, loginForm).then(function (res) {
      if (res.code === 0) {
        ...
        setCookie('BAccessToken', data.accessToken, expiresTime);
...
function NewsModal(_ref) {
  ...
  return /*#__PURE__*/react.createElement(modal/* default */.A, {
    ...
    className: "modal_title"
  }, noticeLatest == null ? void 0 : noticeLatest.title), /*#__PURE__*/react.createElement("div", null, formatTimestamp(noticeLatest == null ? void 0 : noticeLatest.createTime, "YYYY-MM-DD")), /*#__PURE__*/react.createElement("div", {
...
function Comm1() {
  ...
  }, t("Renew Miners"), /*#__PURE__*/react.createElement("i", null, "(", miningFeeTotal, ")"))), /*#__PURE__*/react.createElement("div", {
    className: "comm1_down"
  }, /*#__PURE__*/react.createElement(dist/* NavLink */.k2, {
    to: "https://s3.ap-east-1.amazonaws.com/cdn.goweb3.org/download/app/GoGo_V1.2.1.apk",
    target: "_blank"
  }, t("Download"), " GoGo"), /*#__PURE__*/react.createElement("div", {
    className: "comm1_inva",
    onClick: () => {
      handleCopy("YDgto5X7JqV");
    }
  }, /*#__PURE__*/react.createElement("em", null, t("Invitation code"), ":"), /*#__PURE__*/react.createElement("em", null, "YDgto5X7JqV", /*#__PURE__*/react.createElement(CopyOutlined/* default */.A, {
    style: {
      color: "#fff"
    }
  }))))));
...
function Comm5() {
  ...
  return /*#__PURE__*/react.createElement(comm5_CustomStyle, null, /*#__PURE__*/react.createElement("div", {
    ...
    className: "comm5_all"
  }, "\xA9 2026 BTCS. All rights reserved. ", chain == null ? void 0 : chain.id));
}
...
function ShenmaActivatedModal(_ref) {
  ...
  const [billFeeValue, setBillFeeValue] = (0,react.useState)(12);
...
function Inactive(_ref) {
  ...
  return /*#__PURE__*/react.createElement(react.Fragment, null, !loading ? /*#__PURE__*/react.createElement("div", null, miningList.length > 0 ? miningList.filter(item => {
    // 原有条件：openMintCount > count
    if (!((item == null ? void 0 : item.openMintCount) > (item == null ? void 0 : item.count))) return false;

    // 新增条件：如果是 type === 0 且 coinBalance 等于 0，则不显示
    if ((item == null ? void 0 : item.type) === 0 && Number(coinBalance) === 0) return false;
    return true;
...
function team_main_Main() {
  ...
  const handleUserClub = async () => {
    http2.get(`${axiosUrl}/user/relation/get-club?address=${address}`).then(function (res) {
      if (res.code === 0) {
        const data = res.data;
  ...
  const handleTeamData = async () => {
    http2.get(`${axiosUrl}/user/relation/invite-info?address=${address}`).then(function (res) {
      if (res.code === 0) {
        const data = res.data;
        // 团队总人数
  ...
  const handleReferrerList = async () => {
    setLoading(true);
    http2.get(`${axiosUrl}/user/relation/invite-list?address=${address}&pageNo=${pageNo}&pageSize=${pageSize}`).then(function (res) {
      if (res.code === 0) {
        const data = res.data;
        setLoading(false);
  ...
  }, [address, chain, accessToken, pageNo, pageSize]);
...
/******/ 	var __webpack_exports__ = __webpack_require__.O(undefined, [977], () => (__webpack_require__(5312)))
